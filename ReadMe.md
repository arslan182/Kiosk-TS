...
```shell
    docker history juergenzimmermann/kiosk:2026.4.1-hardened
    docker history juergenzimmermann/kiosk:2026.4.1-trixie
    docker history juergenzimmermann/kiosk:2026.4.1-alpine
```
...
```shell
    docker inspect juergenzimmermann/kiosk:2026.4.1-hardened
    docker inspect juergenzimmermann/kiosk:2026.4.1-trixie
    docker inspect juergenzimmermann/kiosk:2026.4.1-alpine
```
...
```shell
    docker sbom juergenzimmermann/kiosk:2026.4.1-hardened
    docker sbom juergenzimmermann/kiosk:2026.4.1-trixie
    docker sbom juergenzimmermann/kiosk:2026.4.1-alpine
```

### Docker Compose

Mit _Docker Compose_ und der Konfigurationsdatei `compose.yml` im Verzeichnis
`extras\compose\kiosk` lässt sich der Container mit dem "hardened" Basis-Image mit
_Bun_ und _Debian Trixie (13) Slim_ folgendermaßen starten und später in einer
weiteren PowerShell herunterfahren.

```shell
    cd extras\compose\kiosk

    # Shell fuer kiosk-Server mit Trixie-Image zzgl. DB-Server und Mailserver
    docker compose up

    # Nur zur Fehlersuche: weitere Shell für bash/ash bei Trixie oder Alpine
    cd extras\compose\kiosk
    docker compose exec kiosk bash
        id
        env
        exit
    ...
    # Shell: kiosk-Server einschl. DB-Server und Mailserver herunterfahren
    cd extras\compose\kiosk
    docker compose down
```
...
```shell
    docker scout quickview juergenzimmermann/kiosk:2026.4.1-hardened
    docker scout quickview juergenzimmermann/kiosk:2026.4.1-trixie
    docker scout quickview juergenzimmermann/kiosk:2026.4.1-alpine
```
...
```shell
    ...
    docker scout cves --format only-packages juergenzimmermann/kiosk:2026.4.1-hardened
```
